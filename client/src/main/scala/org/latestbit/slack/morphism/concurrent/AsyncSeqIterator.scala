...
/**
 * Provides infinite and async computation results lazily iterating over some user defined function Unlike standard
 * Stream[]/LazyList[] from Scala, this implementation doesn't memorise previous values. Unlike Future.sequence/fold we
 * don't know beforehand how many async actions are coming
 *
...
object AsyncSeqIterator {
  ...
  object cons {
    ...
    /**
     * Constructor of an async iterator over some sequence of futures, generated by a provided function and previous
     * state in a sequence
     *
     * @param initial
     *   initial async item
     * @param toValue
     *   function to extract value from a provided item
     * @param getPos
     *   function to extract the current state or position of current item. When a returned position is None than the
     *   whole sequence is completely finished
     * @param producer
     *   generator of next item based on a state/position in previous item
     * @tparam F
     *   async/effect monad kind (for example standard scala.concurrent.Future)
     * @tparam I
     *   item type
     * @tparam A
     *   value type
