...
object AsyncSeqIterator {
  ...
  object cons {
     ...
     *
     * def initialValue() : Future[MyItem] = ??? // Any implementation
     * def nextMyItem(cursor : String) : Future[MyItem] = ??? // Any implementation
     *
     * val iterator = AsyncSeqIterator.cons (
     ...
     *  },
     *  producer = nextMyItem
     * ) // which is AsyncSeqIterator.cons[MyItem, String, Int] resulting in AsyncSeqIterator[MyItem, String]
     *
     ...
     * @param convValue function to extract value from a provided item
    ...
    def apply[I, A, P](
      ...
      new AsyncSeqIterator[I, A] {
        ...
        override def value()(
            implicit ec: ExecutionContext
        ): Future[A] = computed.map( convValue )
